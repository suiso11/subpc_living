"""VRMアバターモデルの発見と解決 (Phase 6b)。

ユーザー所有VRM/GLBの読み込みを優先し、第三者モデルを製品へ同梱しない
(roadmap §7 / §8 Phase 6)。モデルファイルはリポジトリに置かず、
``models/vrm/`` (gitignore済み) から実行時に発見する。

解決順序:
1. 明示パス引数
2. ``DESKTOP_VRM_MODEL`` 環境変数
3. ``models/vrm/`` 内の ``*.vrm`` / ``*.glb`` を名前順に走査した先頭
"""
...
def discover_vrm_models(directory: Path) -> list[Path]:
    """ディレクトリ内の ``*.vrm`` / ``*.glb`` を名前順で返す。無ければ空リスト。"""
    if not directory.is_dir():
        return []
    return sorted(
        p for p in directory.glob("*")
        if p.suffix.lower() in (".vrm", ".glb") and p.is_file()
    )
