...
| GPU | NVIDIA Tesla P40（VRAM 24GB）※ GTX 1060 3GBから換装予定 |
| RAM | 32GB |
| ストレージ | SSD 512GB + SSD 256GB（合計約768GB） |
| OS | Ubuntu 24.04 LTS |
| 電源ユニット | 500W 🚨 **最大のボトルネック**。P40換装にはPSU交換が事実上必須（後述） |
| PCケース | フルレングスGPU搭載可能 |
...
**現在の500W電源はP40換装における最大のボトルネック。**

#### 電力バジェット（P40換装後の見積もり）

| コンポーネント | TDP / 推定消費 | ピーク時 | 備考 |
|---------------|---------------|---------|------|
| Tesla P40 | 250W | ~250W | 推論フルロード時。nvidia-smi -pl で制限可能だが性能低下 |
| i7-8700 | 65W | ~95W | ターボブースト時 |
| RAM 32GB (DDR4) | ~10W | ~12W | 4枚構成の場合 |
| SSD ×2 | ~10W | ~10W | |
...
- **ピーク負荷407W / 500W = 81%負荷** → PSUの効率低下ゾーン、長時間は危険
- 安価な500W PSUは実際に500W出せないものも多い（12Vレール容量に注意）
- **24時間365日稼働**が前提のため、常時高負荷は寿命・安全性に直結
- P40の突発的な電力スパイクで保護回路が動作→システムダウンのリスク
- GPU省電力（nvidia-smi -pl 200等）で緩和可能だが、推論性能が犠牲になる

#### 対策

| 対策 | 効果 | コスト | 備考 |
|------|------|--------|------|
| **PSU交換（650W 80PLUS Gold以上）** | ✅ 根本解決 | 5,000〜8,000円 | **P40換装前に実施すべき。最優先** |
| nvidia-smi -pl 200 で電力制限 | △ 応急処置 | 0円 | 推論速度10〜20%低下。根本解決にならない |
| nvidia-smi -pl 150 で大幅制限 | △ 安全だが遅い | 0円 | 性能が大幅劣化。P40の意味が薄れる |

> ⚠️ **結論: P40換装の前に650W以上のPSU交換が事実上必須。** 500Wでの24時間P40運用は推奨できない。
...
- **現行GPUはGTX 1060（3GB）**。LLMは7B Q4が上限、STT/TTS/VisionはCPU実行
- GTX 1060はPascal世代。FP16が実質非対応のため量子化（Q4/Q5）前提
- **P40換装には500W PSUの交換が事実上必須**（ピーク400W超、24時間稼働に耐えない）
...
| ~~RAM増設（16GB→32GB）~~ | ~~推奨~~ | ~~3,000〜5,000円~~ | ✅ **済** |
| **電源ユニット換装（650W 80PLUS Gold）** | **🚨 最優先** | **5,000〜8,000円** | **P40換装の前提条件。500Wでは24時間P40運用は危険** |
